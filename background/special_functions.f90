!*******************************************************************
!
!     Source code from the Zhang and Jin book as found on a web site
!
!     https://people.sc.fsu.edu/~jburkardt/f77_src/special_functions/special_functions.html 
!
!     Used to cross check the output from the C++ version 
!
!*******************************************************************
      program test
!
...
!
      implicit none
!
      integer ( kind = 4 ), parameter :: MM =5  ! the physical dimension of CPM and CPD.
!
      integer ( kind = 4 ), parameter :: M = 5  ! the order of Pmn(z).
      integer ( kind = 4 ), parameter :: N = 5  ! the order of Pmn(z).
!
      integer ( kind = 4 ), parameter :: nx = 50
      integer ( kind = 4 ), parameter :: ny = 50
!
      integer ix, iy, il, im, indx
!
      real ( kind = 8 )    X, Y ! the real and imaginary parts of the argument Z.
!
      real ( kind = 8 ), parameter :: xdelta = 0.1d+00
      real ( kind = 8 ), parameter :: ydelta = 0.1d+00
!
      complex ( kind = 8 ) CPM(0:MM,0:N), CPD(0:MM,0:N) ! the values of Pmn(z) and P'mn(z)
!
!---- Iterate over a square of points avoiding the branch cut
!
      write(6,1045) 
!
      indx = 0
!
      do ix=-nx, nx
         do iy=-ny, ny
            ...
            y = iy * ydelta
!
            if( (x .lt. -0.988e+00) .and. (abs(y) .lt. 0.01e+00) )then
              continue ! ignore branch cut [-\infty,-1] on imaginary axis
            else
...
!
              do im=0,m
                 do il = im,n
                    indx = indx + 1
                    if( (2 .eq. im) .and. (3 .eq. il) )then
                      write(6,1050) indx, il, im, x, y, sqrt(x*x + y*y), cpm(im,il)
                    endif
                 end do
              end do 
            endif 
...
!
      stop
!
 1045 format(5x,'index  l    m              P_lm                     |z|       Associated Legendre function',/,&
             5x,'----- ---  ---  ----------------------------   -------------  ----------------------------')
 1050 format(5x,i6,i3,2x,i3,2x,'(',d13.6,', ',d13.6,')  ',d13.6,2x,'(',d13.6,', ',d13.6,')')
!
...
!
      implicit none
!
      integer ( kind = 4 ), parameter :: MM =5  ! the physical dimension of CQM and CQD.
!
      integer ( kind = 4 ), parameter :: M = 5  ! the degree of Qmn(z).
      integer ( kind = 4 ), parameter :: N = 5  ! the order  of Qmn(z).
!
      integer ( kind = 4 ), parameter :: nx = 50
      integer ( kind = 4 ), parameter :: ny = 50
!
...
!
      ...
      real ( kind = 8 ), parameter :: ydelta = 0.1d+00
!
      complex ( kind = 8 ) CQM(0:MM,0:N), CQD(0:MM,0:N) ! the values of Qmn(z) and Q'mn(z)
!
!---- Iterate over a square of points avoiding the branch cut
!
      write(6,1045) 
...
!
              do im=0,m
                 do il = im,n
                    indx = indx + 1
                    !if( (1 .eq. im) .and. (1 .eq. il) )then
                      if(abs(y) .lt. 0.001)then 
                        if( abs(x - 2.5) < 0.001)then
                          write(6,1050) indx, il, im, x, y, sqrt(x*x + y*y), cqm(im,il)
                        endif
                      endif 
                    !endif
                 end do
...
!
      stop
!
 1045 format(5x,'index  l    m              Q_lm                      |z|        Associated Legendre function',/,&
             5x,'----- ---  ---  ------------------------------  -------------  -----------------------------')
 1050 format(5x,i6,i3,2x,i3,2x,'(',d13.6,', ',d13.6,')  ',d13.6,2x,'(',d13.6,', ',d13.6,')')
!
      end subroutine testQlm
!
      subroutine testQlm2()
!
      implicit none
!
      integer ( kind = 4 ), parameter :: MMAX =5  ! the physical dimension of CQM and CQD.
!
      integer ( kind = 4 ), parameter :: M = 5     ! the degree of Qmn(z).
      integer ( kind = 4 ), parameter :: LMAX = 5  ! the order  of Qmn(z).
!
      integer ( kind = 4 ), parameter :: maxtest = 6
!
      integer ix, iy, il, im, indx
      integer mtop, ntest
!
      integer l_vec_test(maxtest)
      integer m_vec_test(maxtest)             
!
      real ( kind = 8 )    X, Y ! the real and imaginary parts of the argument Z.
!
!---- The associated Legendre functions (CQM) and derivatives (CQD) 
!
      complex ( kind = 8 ) CQM(0:MMAX,0:LMAX)
      complex ( kind = 8 ) CQD(0:MMAX,0:LMAX) 
!
!---- Test values from Zhang and Jin page 118 ff
!
      l_vec_test(1) = 1
      m_vec_test(1) = 2
!
...
!
      mtop = 5
!
!---- Compute matrices (l,m) of cqm and cqd at this z = (x + iy)
!
      x = 2.0e+00 
      y = 0.0e+00
!
      call clqmn ( mtop, mmax, lmax, x, y, cqm, cqd )
!
      indx = 0
      write(6,1045) 
!
...
!---- Format statements
!
 1045 format(5x,'index  l    m          Argument (z)                 |z|        Associated Legendre function',/,&
             5x,'----- ---  ---  ------------------------------  -------------  -----------------------------')
 1050 format(5x,i5,i4,2x,i3,2x,'(',d13.6,', ',d13.6,')  ',d13.6,2x,'(',d13.6,', ',d13.6,')')
!
      end subroutine testQlm2
!
...
!
!--- Set factor for inside, outside unit circle
!
...
!
!---- One element below the diagonal
!
...
! 
    write(6,*) ' Completed cqm(0,k) computations'
!
    cqf2 = 0.0D+00
    cqf1 = 1.0D+00
    do k = km, 0, -1
