...
constexpr unsigned int const real_factorial(unsigned int const x) {
    return (x <= 1) ? 1 : real_factorial(x-1)*x;
}
...
template <typename T>
       ...
       real_factorial(unsigned int const n, T &xfact)
  {
   ...
   std::ostringstream& os = *(std::ostringstream*)(nullptr);
//   std::ostringstream os;

   //
   const static T table[11] = {
        real_factorial(0),
        real_factorial(1),
        real_factorial(2),
        real_factorial(3),
          ...
          std::complex<T> &zretVal)
  {
   ...
   if(m == 0)
   ...
   else
     {
      ...
      std::complex<T> const ztemp2 =
        (m == 1) ? std::sqrt(ztemp) :
        (m == 2) ? ztemp :
        (m == 3) ? ztemp * std::sqrt(ztemp) :
        (m == 4) ? ztemp * ztemp :
        (m == 5) ? ztemp * ztemp * std::sqrt(ztemp) :
                   std::pow(ztemp, xn);
